...
$n_year = $data['n_year'];
$selectedStudent = $data['students'];
$selectedCourse = $data['sentSelectedCourse'];
...
for ($i = 0; $i < count($selectedStudent); $i++) {
    $query = "UPDATE `admin` SET `optional_subject`='$selectedCourse' WHERE id=$selectedStudent[$i];";
...
if (empty($allId) || empty($allStudentId) || empty($allStudentName)) {
    ...
    $sentData['error'] = false;
    $sentData['message'] = 'Updaeted Selected Students Optional Course';
